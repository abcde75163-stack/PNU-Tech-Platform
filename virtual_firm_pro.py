...
def extract_text_from_file(uploaded_file):
    ...
    if file_name.endswith('.pdf'):
        ...
        st.warning(f"지원하지 않는 파일 형식입니다: {file_name}")
        return ""
...
# 템플릿 내의 태그({{태그}})를 찾아 내용을 삽입하는 함수 (폰트 강제 적용 기능 추가)
def replace_placeholder(doc, placeholder, content, is_inline=False, font_name=None, font_size=None, is_bold=False):
    def apply_inline_replace(p):
        original_text = p.text
        p.text = original_text.replace(placeholder, content)
        # 인라인 교체 후 폰트가 지정되어 있다면 적용
...
def run_virtual_firm(spec_file, doc_template, target_corp, ir_data, business_status):
    if not spec_file:
        st.error("분석을 위한 기술 명세서(PDF 또는 Word)가 필요합니다.")
        return
    ...
    with st.spinner("🚀 완벽한 비즈니스 이전을 위한 심층 사업 타당성 및 '기술가치평가 산출'을 진행 중입니다... (약 20~30초 소요)"):
        try:
            ...
            if used_template:
                # 수정됨: tech_title을 교체할 때 KoPub돋움체_Pro Bold, 18pt 강제 지정
                replace_placeholder(
                    doc, "{{tech_title}}", clean_title, 
                    is_inline=True, 
                ...
                set_font(title_run, "KoPub돋움체_Pro Bold", 18, bold=True) # 여기도 18pt로 통일
